...
This directory implements UWSIGN's lightweight batch scheduler for CPU-bound parallel work. The design combines native threads with C++20 coroutine handles, but it is intentionally much smaller than a general-purpose async runtime:
...
In the current codebase this mechanism is a reusable utility for coarse CPU-bound batches. UWSIGN can use it for signing, hashing, verification, or other workflows that prepare all tasks before execution.
...
Callers that need recoverable error handling should follow that model: each task catches expected failures internally and reports them through atomics plus a shared error slot before the caller handles the batch result.

## Current Integration Pattern

The expected production pattern in UWSIGN is fixed-size batch work:

- The caller splits input work into task groups.
...
This model gives UWSIGN parallel batch execution without introducing a heavyweight runtime dependency or a persistent scheduler subsystem.
...
- Good fit for batch-parallel work.
...
That trade-off is intentional: UWSIGN only needs a dependable, low-overhead way to fan out coarse-grained work, and this design stays close to that requirement.
