...
UWSIGN_MODULE_EXPORT namespace uwsign::utils::mutex
{
    ...
    UWSIGN_ALWAYS_INLINE inline constexpr void rwlock_pause() noexcept
    {
...
# if UWSIGN_HAS_BUILTIN(__builtin_ia32_pause)
...
# if UWSIGN_HAS_BUILTIN(__builtin_aarch64_yield)
        __builtin_aarch64_yield();
# elif UWSIGN_HAS_BUILTIN(__builtin_arm_yield)
        __builtin_arm_yield();
# else
        ::std::atomic_signal_fence(::std::memory_order_seq_cst);
# endif
#elif defined(__riscv)
# if UWSIGN_HAS_BUILTIN(__builtin_riscv_pause)
...
# endif
    ...
    UWSIGN_ALWAYS_INLINE inline constexpr void rwlock_assert_no_reader_overflow([[maybe_unused]] unsigned old) noexcept
    {
        ...
        if((old & reader_count_mask) == reader_count_mask) [[unlikely]] { ::uwsign::utils::debug::trap_and_inform_bug_pos(); }
#endif
    }
