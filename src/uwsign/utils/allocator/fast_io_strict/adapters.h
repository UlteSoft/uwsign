...
# endif
    ...
    namespace details
    {
        ...
        {
            static_assert(::uwsign::utils::allocator::fast_io_strict::fast_io_strict_generic_allocator_adapter<alloc>::has_native_allocate);
            ...
            void* p;
            if constexpr(zero) { p = ::uwsign::utils::allocator::fast_io_strict::fast_io_strict_generic_allocator_adapter<alloc>::allocate_zero(n); }
            else
            {
                p = ::uwsign::utils::allocator::fast_io_strict::fast_io_strict_generic_allocator_adapter<alloc>::allocate(n);
            }
            if(p == nullptr) [[unlikely]] { return nullptr; }
        ...
        inline constexpr ::fast_io::allocation_least_result allocator_pointer_aligned_at_least_impl(::std::size_t alignment, ::std::size_t n) noexcept
        {
            ...
            if constexpr(zero) { res = ::uwsign::utils::allocator::fast_io_strict::fast_io_strict_generic_allocator_adapter<alloc>::allocate_zero_at_least(n); }
            else
            {
                res = ::uwsign::utils::allocator::fast_io_strict::fast_io_strict_generic_allocator_adapter<alloc>::allocate_at_least(n);
            }
            if(res.ptr == nullptr) [[unlikely]] { return {nullptr, 0}; }
	...
	inline constexpr void *status_allocator_pointer_aligned_impl(typename alloc::handle_type handle, ::std::size_t alignment, ::std::size_t n) noexcept
	{
		...
		constexpr ::std::size_t defaultalignment{::uwsign::utils::allocator::fast_io_strict::details::calculate_default_alignment<alloc>()};
		bool const alignedadjustment{defaultalignment < alignment};
		if (alignedadjustment)
		{
			n = ::uwsign::utils::allocator::fast_io_strict::details::allocator_compute_aligned_total_size_impl(alignment, n);
		}
		void *p;
		if constexpr (zero)
		{
		...
		if (alignedadjustment)
		{
			p = ::uwsign::utils::allocator::fast_io_strict::details::allocator_adjust_ptr_to_aligned_impl(p, alignment);
		}
		return p;
	}
	...
	inline constexpr ::fast_io::allocation_least_result status_allocator_pointer_aligned_at_least_impl(typename alloc::handle_type handle, ::std::size_t alignment, ::std::size_t n) noexcept
	{
		...
		{
			auto resptr{res.ptr};
			auto aligned_ptr = ::uwsign::utils::allocator::fast_io_strict::details::allocator_adjust_ptr_to_aligned_impl(resptr, alignment);
