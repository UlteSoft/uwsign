...
rule("debug", function()
    on_load(function(target)
        ...
        target:add("defines", "UWSIGN_MODE_DEBUG")
...
rule("release", function()
    on_load(function(target)
        ...
        target:add("defines", "UWSIGN_MODE_RELEASE")
...
rule("minsizerel", function()
    on_load(function(target)
        target:add("defines", "NDEBUG")
        target:add("defines", "UWSIGN_MODE_MINSIZEREL")
...
rule("releasedbg", function()
    on_load(function(target)
        target:add("defines", "NDEBUG")
        target:add("defines", "UWSIGN_MODE_RELEASEDBG")
        target:set("optimize", "fastest")
        --target:set("fpmodels", "precise")
        target:set("symbols", "debug")
        target:set("policy", "build.optimization.lto", enable_lto)
